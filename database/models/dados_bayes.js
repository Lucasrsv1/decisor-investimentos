...
class DadosBayes extends Model {
	/**
 ...
 * @returns
 */
function initDadosBayes (sequelize) {
	DadosBayes.init({
		id: {
		...
		resultado: {
			type: DataTypes.ENUM("R < -1%", "-1% <= R <= 0%", "0% < R <= 1%", "R > 1%"),
			allowNull: false
		},
		sentido: {
			type: DataTypes.ENUM("DESVALORIZACAO", "VALORIZACAO"),
			allowNull: false
		}
	}, {
		sequelize,
		timestamps: false,
		modelName: "DadosBayes",
		tableName: "dados_bayes",
		underscored: true
	});
